            ...
            <div class="nav notify-row" id="top_menu">
                ...
                <ul class="nav top-menu ">
                    ...
                    if ($role == "team1") {
                        ...
                        // Query to count rows with 'active' status
                        $notification_query = "SELECT COUNT(*) as total FROM main_store_request WHERE team1 = ? AND status =?";

                        // Prepare the statement for better security (prevent SQL injection)
                        $stmt = mysqli_prepare($conn, $notification_query);
                        mysqli_stmt_bind_param($stmt, "is", $count, $status);
                        mysqli_stmt_execute($stmt);

                        // Get the result
                        mysqli_stmt_bind_result($stmt, $total_active);
                        ...
                        // Query to count rows with 'active' status
                        $notification_query = "SELECT COUNT(*) as total FROM main_store_request WHERE team2 = ? AND status =?";
                        ...
                        // this is for the admin of the department for reciving request
                        $total_active = 0;
                    }
                  ...
                  <li id="header_inbox_bar" class="dropdown pull-right top-menu">
                        <a href="request.php" title="Main Store Request notification">
                            <i class="fa fa-envelope-o"></i>
                            <span class="badge bg-theme"><?php echo $total_active; ?></span>
                        </a>
                    </li>
                    <li id="header_inbox_bar" class="dropdown pull-right top-menu">
                        <a href="pain_mini_mini_request.php" title="Paint Mini Mini Store Request notification">
                            <i class="fa fa-envelope-o"></i>
                            <span class="badge bg-theme"><?php echo $total_active; ?></span>
                        </a>
                    </li>
        ...
        <section id="main-content">
            <section class="wrapper">
                <div class="row">
                 ...
                 main_store_request.In_out,main_store_request.sender_team2,main_store_request.department,main_store_request.cancel,main_store_request.sender_team1,main_store_request.qnty ,
                 main_store_request.transaction_id,main_store_request.requested_date,main_store_request.InFrom_OutTo,main_store_request.sender_id,
                inventory_items.item_description AS item_description ,inventory_items.item_code AS item_code , inventory_items.uom AS uom
                FROM main_store_request INNER JOIN inventory_items ON main_store_request.item_code = inventory_items.item_code where status='inactive' and cancel='0'";
                                                ...
                                                <?php foreach ($main_store_query_run_data as $request): ?>
                                                    <tr class="attendanceRow">
                                                        ...
                                                        if ($role == "admin") {
                                                            ...
                                                            ?>
                                                            <td>
                                                                <button class="btn btn-primary rejectRequestBtn"
                    ...
                    <div class="modal fade" id="deleteRequestmodal" tabindex="-1" role="dialog"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <form id="deleteForm" action="../assets/php_query/request_query.php" method="post">
                    ...
                    <div class="modal fade" id="AcceptRequestModal" tabindex="-1" role="dialog"
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <form id="acceptForm" action="../assets/php_query/request_query.php" method="post">
                                    <div class="modal-header">
